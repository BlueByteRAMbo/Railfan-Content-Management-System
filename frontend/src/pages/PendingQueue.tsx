          ...
          <div className="divide-y divide-white/5">
            ...
            {page?.content.map(v => (
              <div key={v.id} className={`p-4 flex flex-col md:flex-row gap-4 items-center hover:bg-white/5 transition-colors ${selectedIds.includes(v.id) ? 'bg-brand-500/5' : ''}`}>
                ...
                {/* Thumbnail */}
                <div className="w-full md:w-40 h-24 bg-slate-800 rounded-lg overflow-hidden flex-shrink-0 cursor-pointer" onClick={() => navigate(`/videos/${v.id}`)}>
                  {v.thumbnail ? (
                    <img src={v.thumbnail.startsWith('http') ? v.thumbnail : `data:image/jpeg;base64,${v.thumbnail}`} alt="" className="w-full h-full object-cover hover:scale-105 transition-transform" />
                  ) : (
                ...
                <div className="w-full md:w-auto flex gap-2">
                  ...
                  <button 
                    ...
                    className="btn-primary flex-1 md:flex-none py-2 px-4 whitespace-nowrap flex items-center justify-center"
                  >
